  ...
  def spec =
    suite("todo-conduit")(
      test("add, toggle, filter, and clear completed"):
        PreviewServe.withServer(Preview.routes(PreviewConfig(Repo.preview("todo-conduit")))): base =>
          (for
            page <- Chekhov.page
          ...
          yield assertTrue(list.contains("buy milk"))).tapError(_ => screenshot("todo-conduit"))
